...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            sanskariresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            sanskarifile = await event.client.download_media(sanskariresponse, "./temp")
            return await unzip(sanskarifile)
